...
Atmos Toolkit 是一个 Python GFS 多气象变量数据获取与地图可视化工具。它从 NOAA GFS（Global Forecast System）0.25° 预报数据集下载 16 种气象变量（风场 U/V、温度、湿度、云量、能见度、降水、气压、阵风、位势高度等）：标量变量生成暗色主题的气象地图 PNG（数据色斑 + 海岸线国界 + colorbar）并切割为透明 XYZ 瓦片供 Web 地图叠加，风场则生成粒子流 JSON 数据供前端渲染动态风效。Python 3.12+，使用 uv 管理依赖。
...
2. **地图生成（标量变量）** (`processor.py` + `map_visualizer.py`): 加载 NetCDF → 单位转换 → 逐帧调用 `generate_scalar_map()` 生成暗色主题气象地图 PNG，输出到 `outputs/textures/{variable}/{level_token}/`
3. **瓦片生成（标量变量）** (`tile_generator.py`): 将数据色斑生成透明 RGBA 叠加层后，PlateCarree 重投影切割为 Web Mercator XYZ 瓦片，缩放级别 3-4，输出到 `atmos-tiles/{variable}/{level_token}/{z}/{x}/{y}/{unix_timestamp}.png`
4. **风场粒子数据** (`wind_data_generator.py`): 风场（vector）从 U/V 分量生成 wind-layer 兼容粒子流 JSON，输出到 `atmos-tiles/wind/{level_token}/particle/{unix_timestamp}.json`
5. **瓦片清单** (`tile_generator.py`): 每个 (variable, level_token) 组合生成 `atmos-tiles/{variable}/{level_token}/tiles_manifest.json`
...
| `map_visualizer.py` | 暗色主题气象地图渲染（标量变量）：通用辅助函数 + generate_scalar_map |
...
- **风场/标量分流**: 风场（vector）仅生成粒子流 JSON（`wind_data_generator`），标量变量走 `generate_scalar_map/tiles`，共享底图/海岸线/网格/标题/colorbar 辅助函数
...
- **智能调度**: `--schedule` 模式启动立即执行一次，随后按 GFS 周期（00/06/12/18 UTC + 延迟）自动运行；通过 `SCHEDULE_VARIABLES` 环境变量配置多变量调度（默认 wind）
...
- **透明 RGBA 瓦片（仅标量）**: 从原始数据直接生成透明瓦片（仅数据色斑，无底图/标签），通过 `_build_scalar_overlay()` 生成 PlateCarree 叠加层再切割为 Web Mercator 瓦片
...
- **风场不再输出地图/瓦片**: 风场（`wind`）移除暗色主题 PNG 地图（`outputs/textures/wind/`）与 XYZ 瓦片（`atmos-tiles/wind/{level}/{z}/{x}/{y}/`），仅保留粒子流 JSON（`atmos-tiles/wind/{level}/particle/`）。前端风场改用粒子数据渲染。
