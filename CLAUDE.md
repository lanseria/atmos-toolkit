...
Wind Toolkit 是一个 Python GFS 多气象变量数据获取与地图可视化工具。它从 NOAA GFS（Global Forecast System）0.25° 预报数据集下载 16 种气象变量（风场 U/V、温度、湿度、云量、能见度、降水、气压、阵风、位势高度等），生成暗色主题的气象地图 PNG（数据色斑 + 海岸线国界 + colorbar；风场含风向箭头），并切割为透明 XYZ 瓦片供 Web 地图叠加使用。Python 3.12+，使用 uv 管理依赖。
...
# 命令行工作流
python -m src.wind_toolkit.main                       # 默认风场所有层
python -m src.wind_toolkit.main -v temp --level 850    # 温度 850 hPa
python -m src.wind_toolkit.main -v temp                # 温度所有层级（8 等压面 + 2m）
python -m src.wind_toolkit.main --variables wind tcdc vis  # 多变量批量
python -m src.wind_toolkit.main --all-variables        # 全部 16 个变量
...
3. **瓦片生成** (`tile_generator.py`): 将数据色斑生成透明 RGBA 叠加层后，PlateCarree 重投影切割为 Web Mercator XYZ 瓦片，缩放级别 3-4，输出到 `wind-tiles/{variable}/{level_token}/{z}/{x}/{y}/{unix_timestamp}.png`
4. **瓦片清单** (`tile_generator.py`): 每个 (variable, level_token) 组合生成 `wind-tiles/{variable}/{level_token}/tiles_manifest.json`
...
- 按 **变量 + 层级 token** 分隔的路径：`data/raw/{variable}/{level_token}/`、`data/processed/{variable}/{level_token}/`、`outputs/textures/{variable}/{level_token}/`、`wind-tiles/{variable}/{level_token}/`
...
- **wind-tiles 目录结构变更**: 从 `wind-tiles/{hpa}hPa/` 改为 `wind-tiles/wind/{hpa}hPa/`，所有变量统一为 `wind-tiles/{variable}/{level_token}/`。前端 URL 模板需同步更新。
